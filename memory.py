...
class Memory:
    ...
    async def create_desire(
        ...
        HARD FILTER: Reject demonstration/test desires to prevent malformed entries.
        Every cycle without this filter wastes resources on test/demo patterns.
        ...
        Returns:
            The desire ID if created, None if filtered as demonstration desire
        """
        # HARD FILTER: Reject demonstration desires at entry point
        if self._is_demonstration_desire(description):
            logger.warning(f"🚫 Hard filter rejected demonstration desire: {description[:60]}...")
            return None
